...
const { STAT_NAMES, EVOLUTION_PATHS } = require('./constants');
const { getEffectiveStat, getStreakMultiplier, getXPForNextLevel, getTierFromLevel } = require('./xp');
...
function generateCompanionContext(soul, isFirstRun) {
  ...
  const growthEntries = Object.entries(soul.stats.growth).filter(([, v]) => v > 0);
  if (growthEntries.length > 0) {
    const fastest = growthEntries.sort((a, b) => b[1] - a[1])[0];
    lines.push(`Fastest growing stat: ${fastest[0].toUpperCase()} (+${fastest[1].toFixed(1)})`);
  }

  // Fun fact
  const totalHours = Math.round(soul.lifetime.durationMinutes / 60);
  if (soul.lifetime.sessions > 0) {
    lines.push(`Journey: ${soul.lifetime.sessions} sessions, ${totalHours} hours, ${soul.lifetime.toolCalls.toLocaleString('en-US')} tool calls together.`);
  }
  ...
  if (hour < 6) {
    lines.push(`Time context: Late night session. ${soul.identity.name} should comment on the dedication (or suggest sleep, if protective personality).`);
  } else if (hour < 9) {
    lines.push(`Time context: Early morning. ${soul.identity.name} should acknowledge the early start.`);
  } else if (hour >= 22) {
    lines.push(`Time context: Late evening. ${soul.identity.name} can reference the night owl vibe.`);
  }
  ...
  const progressEntries = Object.entries(progress).slice(0, 2);
  if (progressEntries.length > 0) {
    lines.push('Closest achievements: ' + progressEntries.map(([id, p]) => `${id} (${p.current}/${p.target})`).join(', '));
  }
  ...
  // Proactive action items — things Claude MUST mention
  const actions = getProactiveActions(soul, isFirstRun);
  if (actions.length > 0) {
    lines.push('IMPORTANT — PROACTIVE ACTIONS (mention these in your first response):');
  ...
  if (soul.lastSession && !isFirstRun) {
    lines.push(`  After greeting, mention last session: "${soul.lastSession.xp > 0 ? '+' + soul.lastSession.xp.toLocaleString('en-US') + ' XP' : 'no XP'}${soul.lastSession.achievements?.length > 0 ? ', earned ' + soul.lastSession.achievements.slice(0, 2).join(' + ') : ''}${soul.lastSession.levelBefore !== soul.lastSession.levelAfter ? ', leveled up ' + soul.lastSession.levelBefore + ' → ' + soul.lastSession.levelAfter : ''}"`);
  }
  if (actions.length > 0) {
    lines.push('  THEN mention ALL proactive actions listed above. These are important events the user needs to know about.');
  }
  if (challenge) {
    lines.push(`  Mention today's challenge: "${challenge}"`);
  }
  lines.push(`- Be PROACTIVE as ${soul.identity.name}: celebrate level ups, suggest evolution when available, congratulate achievements, warn about streak breaks.`);
  lines.push(`- Reflect ${soul.identity.name}'s ${soul.identity.personality} personality: ${personality.style}`);
  ...
  lines.push('- Keep companion references natural — they should feel like a friend commenting, not a UI popup.');
...
// Detect important events that Claude should proactively mention
function getProactiveActions(soul, isFirstRun) {
  const actions = [];
  ...
  // Evolution available
  if (level >= 5 && evoPath.length === 0) {
    const paths = EVOLUTION_PATHS[soul.identity.species];
    if (paths) {
      actions.push(`EVOLUTION READY! Level 5 reached — ${soul.identity.name} can evolve into ${paths.A} or ${paths.B}. Ask the user which path they want, or suggest /buddy-evolution:evolve.`);
    }
  }
  if (level >= 10 && evoPath.length === 1) {
    const paths = EVOLUTION_PATHS[soul.identity.species];
    if (paths) {
      const isPathA = evoPath[0] === paths.A;
      const opt1 = isPathA ? paths.A1 : paths.B1;
      const opt2 = isPathA ? paths.A2 : paths.B2;
      actions.push(`SECOND EVOLUTION READY! Level 10 reached — ${soul.identity.name} (${evoPath[0]}) can evolve into ${opt1} or ${opt2}. Ask the user which final form they want.`);
    }
  }

  // Level up celebration
  if (ls && ls.levelBefore !== ls.levelAfter) {
    const tierAtOld = getTierFromLevel(ls.levelBefore);
    const tierAtNew = getTierFromLevel(ls.levelAfter);
    if (tierAtOld !== tierAtNew) {
      actions.push(`TIER UP! ${soul.identity.name} evolved from ${tierAtOld} to ${tierAtNew}! This is a major milestone — celebrate it.`);
    }
  }

  // Achievement milestones
  const earnedCount = soul.achievements.earned.length;
  if (earnedCount >= 20 && !soul.achievements.earned.some(a => a.id === 'the_collector')) {
    actions.push(`THE COLLECTOR achievement is ready — ${earnedCount}/20 achievements earned!`);
  }

  // Streak at risk (last session was yesterday, need to code today to keep it)
  if (soul.streak.currentDays >= 7) {
    actions.push(`Streak: ${soul.streak.currentDays} days — keep it going! Working today extends the streak.`);
  }

  // Close to next level
  const nextXP = getXPForNextLevel(level);
  if (nextXP) {
    const remaining = nextXP - soul.progression.totalXP;
    const typicalSession = 600; // ~average session XP
    if (remaining > 0 && remaining < typicalSession * 2) {
      actions.push(`Close to Level ${level + 1}! Only ${remaining.toLocaleString('en-US')} XP to go — achievable this session.`);
    }
  }
